...
tspec = pandas.read_csv(os.path.join(basepath,'transitspec_'+datestr+os.extsep+'csv'),comment="#")
espec = pandas.read_csv(os.path.join(basepath,'emissionspec_'+datestr+os.extsep+'csv'),comment="#")
...
offs =        [(4,-3),  (-5,-16), (4,-8),(4,-4),(6,-4),   (5,-2),   (0,-15),  (-5,4),    (0,0)]
...
def plot1(data, earthmassonly=False,title=None):
    ...
    for m,s,c,o in zip(methods,syms,cmap,methodorder):
        ...
        if earthmassonly:
            mj /=  Ms[2]
        ax.scatter(sma,mj,marker=s,s=60,zorder=o,
    ...
    if earthmassonly:
        ...
        tmp = Ms
    ax.scatter(smas,tmp,marker='o',s=60,facecolors='yellow',edgecolors='k',alpha=1,\
...
def plot2(data, earthmassonly=False,title=None):
    ...
    for m,s,c,o in zip(methods,syms,cmap,methodorder):
        ...
        if m == 'Imaging':
            ...
            ax.scatter(sma.loc[nspecs>0],mj.loc[nspecs>0],marker=s,s=60,zorder=o,
                    facecolors=c,edgecolors='k',alpha=0.95,label=m)
            ax.scatter(sma.loc[nspecs==0],mj.loc[nspecs==0],marker=s,s=60,zorder=-1,
                    facecolors=c,edgecolors='k',alpha=0.1,label=None)
...
plot2(data,earthmassonly=True,title='2020: {} planets with spectroscopic measurements'.format(np.where(data['pl_nespec']+ data['pl_ntranspec'] > 0)[0].size))
plot2(data2,earthmassonly=True,title='2010: {} planets with spectroscopic measurements'.format(np.where(data2['pl_nespec']+ data2['pl_ntranspec'] > 0)[0].size))
