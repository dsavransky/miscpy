...
def nbodyRKN1210(tspan0, np.ndarray[DTYPE_t, ndim=1] y0,
                 np.ndarray[DTYPE_t, ndim=1] dy0, 
                 np.ndarray[DTYPE_t, ndim=1] mus):
    """
    nbodyRKN1210: Runge-Kutta-Nystrom integration of N-body systems.

    Args:
        tspan0 (number or iterable of numbers):
            Integration time span.  If scalar then assumed to be 
            stopping time (with integration from time 0). If more than 
            2 elements, outputs will be generated at each time.
        y0 (float array with 3N elements):
            Initial body positions [x1,y1,z1,x2,y2,z2,...xn,yn,zn]
        dy0 (float array with 3N elements):
            Initial body velocities [dx1,dy1,dz1,...dxz,dyn,dzn]
        mus (float array with N elements):
            Gravitational parameters of bodies.  All units must be
            consistent between y0, dy0 and mus.

    Returns:
        tspan (float ndarray):
            Times of outputs
        yhist (float 2d ndarry):
            Positions at each time step
        dyhist (float 2d ndarray):
